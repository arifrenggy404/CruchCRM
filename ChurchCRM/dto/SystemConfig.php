...
class   SystemConfig
{
    ...
    private static function buildConfigs(): array
    {
        return [
            ...
            'sLanguage'                            => new ConfigItem('sLanguage', 'choice', 'id_ID', gettext('Internationalization (I18n) support'), 'https://poeditor.com/join/project?hash=RABdnDSqAt', json_encode(SystemConfig::getSupportedLocales(), JSON_THROW_ON_ERROR)),
            ...
            'sTaxReport1'                          => new ConfigItem('sTaxReport1', 'text', 'Surat ini menunjukkan catatan pembayaran Anda untuk', gettext('Verbage for top line of tax report. Dates will be appended to the end of this line.')),
            'sTaxReport2'                          => new ConfigItem('sTaxReport2', 'text', 'Terima kasih atas bantuan Anda. Kami sangat menghargai pemberian Anda!', gettext('Verbage for bottom line of tax report.')),
            'sTaxReport3'                          => new ConfigItem('sTaxReport3', 'text', 'Jika Anda memiliki pertanyaan atau koreksi, silakan hubungi gereja di nomor di atas selama jam kerja, Senin-Jumat pukul 09.00-16.00.', gettext('Verbage for bottom line of tax report.')),
            'sTaxSigner'                           => new ConfigItem('sTaxSigner', 'text', '', gettext('Tax Report signer')),
            'sReminder1'                           => new ConfigItem('sReminder1', 'text', 'Surat ini menunjukkan catatan janji dan pembayaran Anda untuk tahun anggaran', gettext('Verbage for the pledge reminder report')),
            'sReminderSigner'                      => new ConfigItem('sReminderSigner', 'text', '', gettext('Pledge Reminder Signer')),
            'sReminderNoPledge'                    => new ConfigItem('sReminderNoPledge', 'text', 'Janji: Kami tidak memiliki catatan janji dari Anda untuk tahun anggaran ini.', gettext('Verbage for the pledge reminder report - No record of a pledge')),
            'sReminderNoPayments'                  => new ConfigItem('sReminderNoPayments', 'text', 'Pembayaran: Kami tidak memiliki catatan pembayaran dari Anda untuk tahun anggaran ini.', gettext('Verbage for the pledge reminder report - No record of payments')),
            'sConfirm1'                            => new ConfigItem('sConfirm1', 'text', 'Surat ini menampilkan informasi yang kami miliki dalam database kami mengenai keluarga Anda. Harap periksa, tandai jika ada perubahan, dan kembalikan formulir ini ke kantor gereja.', gettext('Verbage for the database information confirmation and correction report')),
            'sConfirm2'                            => new ConfigItem('sConfirm2', 'text', 'Terima kasih banyak atas bantuan Anda dalam memperbarui informasi ini.', gettext('Verbage for the database information confirmation and correction report')),
            'sConfirm3'                            => new ConfigItem('sConfirm3', 'text', '', gettext('Verbage for the database information confirmation and correction report')),
            'sConfirm4'                            => new ConfigItem('sConfirm4', 'text', '[  ] Saya tidak lagi ingin terhubung dengan gereja (centang di sini untuk dihapus dari catatan kami).', gettext('Verbage for the database information confirmation and correction report')),
            ...
            'sPledgeSummary1'                      => new ConfigItem('sPledgeSummary1', 'text', 'Ringkasan janji dan pembayaran untuk tahun anggaran', gettext('Verbage for the pledge summary report')),
            'sPledgeSummary2'                      => new ConfigItem('sPledgeSummary2', 'text', ' per', gettext('Verbage for the pledge summary report')),
            'sDirectoryDisclaimer1'                => new ConfigItem('sDirectoryDisclaimer1', 'text', "Setiap upaya telah dilakukan untuk memastikan keakuratan direktori ini. Jika ada kesalahan atau kekurangan, silakan hubungi kantor gereja.\n\nDirektori ini digunakan untuk keperluan jemaat", gettext('Verbage for the directory report')),
            'sDirectoryDisclaimer2'                => new ConfigItem('sDirectoryDisclaimer2', 'text', ', dan informasi yang terkandung di dalamnya tidak boleh digunakan untuk keperluan bisnis atau komersial.', gettext('Verbage for the directory report')),
            'bDirLetterHead'                       => new ConfigItem('bDirLetterHead', 'text', '../Images/church_letterhead.jpg', gettext('Church Letterhead path and file')),
            'sZeroGivers'                          => new ConfigItem('sZeroGivers', 'text', 'Surat ini menunjukkan catatan pembayaran Anda untuk', gettext('Verbage for top line of tax report. Dates will be appended to the end of this line.')),
            'sZeroGivers2'                         => new ConfigItem('sZeroGivers2', 'text', 'Terima kasih atas bantuan Anda. Kami sangat menghargai pemberian Anda!', gettext('Verbage for bottom line of tax report.')),
            'sZeroGivers3'                         => new ConfigItem('sZeroGivers3', 'text', 'Jika Anda memiliki pertanyaan atau koreksi, silakan hubungi gereja di nomor di atas selama jam kerja, Senin-Jumat pukul 09.00-16.00.', gettext('Verbage for bottom line of tax report.')),
            ...
            'sConfirmSincerely'                    => new ConfigItem('sConfirmSincerely', 'text', 'Hormat kami', gettext('Used to end a letter before Signer')),
            'sDear'                                => new ConfigItem('sDear', 'text', 'Kepada Yth.', gettext('Text before name in emails/reports')),
